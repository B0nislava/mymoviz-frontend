...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-flame.vercel.app/movies')
    .then (response => response.json())
    .then(data => {
     console.log(data); setMoviesAPI(data.movies);
    });
  }, []);
  ...
  const movies = moviesAPI.map((data, i) => {
    const isLiked = likedMovies.some(movie => movie === data.title);
    //  const urlImg = `https://image.tmdb.org/t/p/w500/${data.poster_path}`
    //  const reduceOverview = `${data.overview.slice(0, 249)} ...`
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={`${data.overview.slice(0, 249)} ...`} poster={`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
